...
local options = {
	...
	network = false,
	audio = false,
	direct_io = false, -- Windows only: use native Windows API to write to pipe (requires LuaJIT)
...
local function info(w, h)
	...
	disabled = (w or 0) == 0
		...
		or (short_video and options.min_duration > 0)
...
local function quality_fin()
	...
	elseif quality == 2 then
		...
		if mp.get_property_number("video-params/sig-peak", 1) > 1 then
			vf_str = vf_str_pre
				.. ":flags=bicublin,format=fmt=gbrapf32,zscale=t=linear:npl=203,tonemap=tonemap=hable:desat=0.0,zscale=p=709:t=709:m=709,"
				.. vf_str_suffix
		end
	elseif quality == 3 then
		if mp.get_property_number("video-out-params/max-luma", 1) > 203 then
			vf_str = "lavfi=[libplacebo=w="
				.. effective_w
				.. ":h="
				.. effective_h
				.. ":colorspace=bt709:color_primaries=bt709:color_trc=bt709:tonemapping=hable:format=bgra]"
...
local function spawn(time)
	...
	local args = {
		...
		"--vd-lavc-fast",
		"--vd-lavc-threads=" .. options.sw_threads,
		"--hwdec=" .. options.hwdec,
...
local function watch_changes()
	if not dirty or not properties["video-params"] then
		return
	end
	dirty = false
