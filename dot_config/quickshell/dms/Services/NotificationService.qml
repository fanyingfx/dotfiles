...
Singleton {
    ...
    // Activate the app behind a notification: prefer the freedesktop "default"
    // action, then the first explicit action, and finally fall back to
    // launching/focusing the originating app via its desktop entry so a click
    // on a plain notification still jumps to the corresponding application.
    function activateNotification(wrapper) {
        if (!wrapper || !wrapper.notification)
            return false;
        ...
        for (let i = 0; i < actions.length; ++i) {
            const a = actions[i];
            if (a && (a.identifier === "default" || a.identifier === "")) {
                try {
                    a.invoke();
                } catch (e) {}
                return true;
        ...
        let entryId = (wrapper.desktopEntry || "").trim();
        let entry = entryId ? DesktopEntries.heuristicLookup(entryId) : null;
        if (!entry && entryId.endsWith(".desktop"))
            entry = DesktopEntries.heuristicLookup(entryId.slice(0, -8));
        let appName = "";
        if (!entry) {
            appName = (wrapper.appName || "").trim();
            if (appName && appName !== "app")
                entry = DesktopEntries.heuristicLookup(appName);
        }

        // Prefer focusing an existing window over launching a second instance.
        if (_focusExistingAppWindow(entryId, appName || wrapper.appName))
            return true;
        ...
        if (entry) {
            try {
                ...
                log.warn("Failed to launch app from notification:", entry.id || entry.execString || entry.command, e);
            }
        }
        return false;
    }

    // On niri, jump to a running window of the app (switch workspace if needed)
    // instead of spawning a duplicate instance.
    function _focusExistingAppWindow(desktopEntry, appName) {
        if (!CompositorService.isNiri || !NiriService || !NiriService.windows)
            return false;

        const candidates = [];
        const entry = (desktopEntry || "").trim().toLowerCase().replace(/\.desktop$/, "");
        if (entry)
        ...
        const matches = (id, candidate) => {
            if (id === candidate)
                return true;
            // "com.mitchellh.ghostty" matches "ghostty"; "code" matches "com.visualstudio.code".
            return id.startsWith(candidate + ".") || id.endsWith("." + candidate);
        };
            ...
            for (const c of candidates) {
                if (matches(id, c)) {
                    try {
                        ...
                        log.warn("Failed to focus window from notification:", w.app_id, e);
                    }
                }
            }
        }
